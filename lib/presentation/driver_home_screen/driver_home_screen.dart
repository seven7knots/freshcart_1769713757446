...
class _DriverHomeScreenState extends State<DriverHomeScreen> {
  ...
  Future<void> _loadDriverData() async {
    ...
    try {
      ...
      if (userId == null) { _navigateToLogin(); return; }

      final userData = await SupabaseService.client
          .from('users').select('full_name').eq('id', userId).single();

      final driverData = await SupabaseService.client
          .from('drivers').select('id, is_online').eq('user_id', userId).maybeSingle();

      if (driverData == null) {
        setState(() { _driverName = userData['full_name'] ?? 'Driver'; _isLoading = false; });
        return;
      }
      ...
      final earningsData = await SupabaseService.client
          .from('deliveries').select('driver_earnings')
      ...
      final completedCount = await SupabaseService.client
          .from('deliveries').select('id').eq('driver_id', driverId).eq('status', 'delivered')
          .gte('delivery_time', startOfDay.toIso8601String());

      final assignedOrders = await SupabaseService.client.from('orders').select('''
  ...
  void _navigateToLogin() => Navigator.of(context).pushReplacementNamed(AppRoutes.driverLogin);
  ...
  Future<void> _toggleOnlineStatus() async {
    ...
    try {
      ...
      await SupabaseService.client.from('drivers').update({'is_online': newStatus}).eq('user_id', userId);
  ...
  Future<void> _updateOrderStatus(String orderId, String newStatus) async {
    ...
    try {
      ...
      await SupabaseService.client.from('orders').update(updates).eq('id', orderId);

      // Update deliveries table too
      try {
        if (newStatus == 'picked_up') {
          await SupabaseService.client.from('deliveries')
              .update({'status': 'picked_up', 'pickup_time': DateTime.now().toIso8601String()})
              .eq('order_id', orderId);
        } else if (newStatus == 'delivered') {
          await SupabaseService.client.from('deliveries')
              .update({'status': 'delivered', 'delivery_time': DateTime.now().toIso8601String()})
              .eq('order_id', orderId);
        }
      } catch (_) {
        // deliveries record may not exist yet, that's ok
      }

      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Order ${newStatus.replaceAll('_', ' ')}'), backgroundColor: Colors.green));
  ...
  Future<void> _openNavigation(String address, double? lat, double? lng) async {
    final targetLat = lat ?? 33.8886;
    final targetLng = lng ?? 35.4955;
    final url = Uri.parse('https://www.google.com/maps/dir/?api=1&destination=$targetLat,$targetLng');
  ...
  Future<void> _handleLogout() async {
    ...
    if (confirm == true) {
      ...
      if (mounted) Navigator.of(context).pushReplacementNamed(AppRoutes.driverLogin);
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.analytics_outlined),
              onPressed: () => Navigator.pushNamed(context, AppRoutes.driverPerformanceDashboard)),
          ...
          : RefreshIndicator(
              ...
              child: SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  OnlineToggleWidget(isOnline: _isOnline, onToggle: _toggleOnlineStatus),
                  SizedBox(height: 3.h),
                  EarningsCardWidget(todayEarnings: _todayEarnings, completedDeliveries: _completedDeliveries),
                  SizedBox(height: 3.h),
                  DeliveryStatsWidget(
                    completedToday: _completedDeliveries, activeHours: 0,
                    averagePerDelivery: _completedDeliveries > 0 ? _todayEarnings / _completedDeliveries : 0.0),
                  SizedBox(height: 3.h),
  ...
  Widget _buildAssignedOrdersSection(ThemeData theme) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Text('Assigned Orders', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
        const Spacer(),
        if (_assignedOrders.isNotEmpty)
          Text('${_assignedOrders.length}', style: theme.textTheme.titleSmall?.copyWith(
  ...
  Widget _buildEmptyState(ThemeData theme) {
    return Container(
      ...
      child: Column(children: [
        ...
        SizedBox(height: 2.h),
        Text('No Assigned Orders', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
        SizedBox(height: 0.5.h),
        Text('New orders will appear here when assigned to you',
            style: theme.textTheme.bodySmall?.copyWith(color: theme.colorScheme.onSurfaceVariant), textAlign: TextAlign.center),
      ]),
    );
  }
  ...
  Widget _buildOrderCard(ThemeData theme, Map<String, dynamic> order) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: theme.colorScheme.surface, borderRadius: BorderRadius.circular(14),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('Order #$orderNumber', style: theme.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
        SizedBox(height: 1.h),

        // Payment method
        Row(children: [
          Icon(Icons.payment, size: 15, color: theme.colorScheme.onSurfaceVariant),
          SizedBox(width: 1.w),
          Text(paymentMethod == 'cash_on_delivery' ? '💵 Cash on Delivery' :
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              SizedBox(width: 1.w),
              Text('PICKUP', style: TextStyle(fontSize: 9.sp, fontWeight: FontWeight.w700, color: Colors.blue)),
            ]),
            SizedBox(height: 0.5.h),
            Text(storeName, style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600)),
            if (storeAddress.isNotEmpty)
              Text(storeAddress, style: theme.textTheme.bodySmall?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
          ]),
        ),
        SizedBox(height: 1.h),
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              SizedBox(width: 1.w),
              Text('DELIVER TO', style: TextStyle(fontSize: 9.sp, fontWeight: FontWeight.w700, color: Colors.green)),
            ]),
            SizedBox(height: 0.5.h),
            Text(customerName, style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600)),
            Text(deliveryAddress, style: theme.textTheme.bodySmall?.copyWith(color: theme.colorScheme.onSurfaceVariant),
        ...
        Row(children: [
          ...
          Expanded(child: OutlinedButton.icon(
            onPressed: () => _openNavigation(
                isAssigned ? storeAddress : deliveryAddress, deliveryLat, deliveryLng),
            icon: const Icon(Icons.navigation, size: 16),
            label: Text(isAssigned ? 'Navigate to Store' : 'Navigate to Customer', style: TextStyle(fontSize: 10.sp)),
            style: OutlinedButton.styleFrom(padding: EdgeInsets.symmetric(vertical: 1.h)),
          )),
          SizedBox(width: 2.w),
          // Call customer
          IconButton(
        ...
        if (isAssigned)
          SizedBox(width: double.infinity, child: ElevatedButton.icon(
            onPressed: _isUpdatingOrder ? null : () => _updateOrderStatus(orderId, 'picked_up'),
            icon: _isUpdatingOrder ? const SizedBox(width: 16, height: 16,
          ...
          SizedBox(width: double.infinity, child: ElevatedButton.icon(
            onPressed: _isUpdatingOrder ? null : () => _confirmDelivery(orderId, total, paymentMethod),
            icon: _isUpdatingOrder ? const SizedBox(width: 16, height: 16,
                child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
        ...
        if (createdAt != null) ...[
          SizedBox(height: 1.h),
          Text('Ordered: ${createdAt.day}/${createdAt.month} at ${createdAt.hour}:${createdAt.minute.toString().padLeft(2, '0')}',
  ...
  Future<void> _confirmDelivery(String orderId, double total, String paymentMethod) async {
    if (paymentMethod == 'cash_on_delivery') {
      final confirm = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
        title: const Text('Cash on Delivery'),
        content: Text('Confirm you collected \$${total.toStringAsFixed(2)} in cash from the customer.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
          ElevatedButton(
